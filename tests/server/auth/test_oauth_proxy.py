...
class TestOAuthProxyComprehensive:
    ...
    async def test_authorize_forwards_pkce_parameters(self, oauth_proxy):
        """Test that PKCE parameters are forwarded to upstream authorization server."""
        client = OAuthClientInformationFull(
            client_id="test-client",
            client_secret="test-secret",
            redirect_uris=[AnyUrl("http://localhost:54321/callback")],
        ...
        # Test with both code_challenge and code_challenge_method
        params = AuthorizationParams(
            redirect_uri=AnyUrl("http://localhost:54321/callback"),
            redirect_uri_provided_explicitly=True,
            state="client-state-123",
        ...
        # Verify transaction still stores PKCE (for later verification)
        txn_id = query_params["state"][0]
        transaction = oauth_proxy._oauth_transactions.get(txn_id)
        assert transaction is not None
        assert (
    ...
    async def test_authorize_forwards_pkce_without_method(self, oauth_proxy):
        """Test that PKCE challenge is forwarded even without explicit method (defaults to S256)."""
        client = OAuthClientInformationFull(
            client_id="test-client",
            client_secret="test-secret",
            redirect_uris=[AnyUrl("http://localhost:54321/callback")],
        ...
        # Test with only code_challenge (no explicit method)
        params = AuthorizationParams(
            redirect_uri=AnyUrl("http://localhost:54321/callback"),
            redirect_uri_provided_explicitly=True,
            state="client-state-123",
